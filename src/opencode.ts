...
export const SpotterPlugin: Plugin = async ({ $, directory }) => {
  ...
  const spotter_exercise = tool({
    ...
    args: {
      ...
      scaffold: tool.schema.string().describe(
  ...
  const commands = {
    ...
    "spotter:off": {
      ...
      template: "Confirm that Spotter gym mode is now off and you will resume writing code normally.",
    },
    "spotter:status": {
    ...
    event: async ({ event }) => {
      ...
      if (cmd === "spotter:done" || cmd === "spotter:solve" || cmd === "spotter:skip") {
        if (state.exercise) {
          ...
          state.exercise = null;
          state.counter = 0;
          try { await $`git -C ${directory} checkout ${orig}`.quiet(); } catch { /* ignore */ }
        }
      }
    },
  };
